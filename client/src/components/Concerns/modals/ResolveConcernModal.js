...
export default function ResolveConcernMOdal(props) {
  ...
  const handleFormSubmit = async() =>{
    axios.post(`${process.env.REACT_APP_BACKEND_URL}/admin/report/resolveConcern/${props.data.concern_id}`,
    {password:values.password,accessToken:Cookies.get("user_id")})
    .then(res=>{
      if(res.data.success){
        refetch();
        enqueueSnackbar(res.data.message, { variant:'success' });
        props.setOpenResolveModal(false);
      }else{
  ...
  return (
    ...
    <Grid item xs={6} marginTop={-2}>
      ...
      <DialogActions>
        ...
        <button className='btn btn-success' disabled={!isValid} type="submit" onClick={handleSubmit}>Resolve</button>
      </DialogActions>
    </BootstrapDialog>
  );
}
