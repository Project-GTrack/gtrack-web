...
export default function InactiveAccountsComponent() {
        ...
        <TableBody>
          {rows.map((row) => (
            <StyledTableRow key={row.id}>
              ...
              <TableCell><Box sx={{ flexGrow: 1, display: { xs: 'none', md: 'flex' } }}><Button onClick={()=>handleOpenModal(row.id)}><FindInPageIcon/></Button><Button color="success" onClick={()=>handleDeleteModal(row.id)}><VisibilityIcon/></Button></Box></TableCell>
              <ViewEmployeeModal data={data} openModal={openModal} setOpenModal={setOpenModal} handleCloseModal={handleCloseModal}/>
              <ReactivateModal data={data} openDeleteModal={openDeleteModal} setDeleteModal={setDeleteModal} handleCloseDeleteModal={handleCloseDeleteModal}/>
            </StyledTableRow>
          ))}
        </TableBody>
      </Table>
